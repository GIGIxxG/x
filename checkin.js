...
/**
 * [步骤1] 刷新 Token
 * (基于 refresh.txt [来源 86, 87, 88])
 */
async function fetchNewTokens(refreshToken) {
    const url = "https://api.welink.huaweicloud.com/mcloud/mag/v7/refresh/LoginReg"; // [来源 86]
    ...
    req.headers = {
        ...
        "uuid": USER_DEVICE_ID,
        "Content-Type": "application/x-www-form-urlencoded" // [来源 86]
    };
    
    // 设置 Body [来源 86]
    // 我们必须对动态的 refresh_token 进行 URL 编码，而 tenantid 已经是编码过的
    req.body = `refresh_token=${encodeURIComponent(refreshToken)}&tenantid=${STATIC_TENANT_ID_ENCODED}&thirdAuthType=3`;

    // 发送请求
    const response = await req.load(); // 使用 .load() 来获取 Headers
    const responseBody = JSON.parse(response.string());
    
    // 从 Headers 中解析 Cookies [来源 87]
    const cookiesHeader = req.response.allHeaderFields['Set-Cookie'];
    if (!cookiesHeader) throw new Error("刷新失败：未找到 Set-Cookie。");

    const token = cookiesHeader.match(/token=([^;]+);/)[1];
    const cdnToken = cookiesHeader.match(/cdn_token=([^;]+);/)[1];
    const hwafSESID = cookiesHeader.match(/HWWAFSESID=([^;]+);/)[1];
    const hwafSESTIME = cookiesHeader.match(/HWWAFSESTIME=([^;]+);/)[1];
    
    // 从 Body 中解析新的 refresh_token [来源 88]
    const newRefreshToken = responseBody.refresh_token;

    if (!token || !newRefreshToken) {
        throw new Error("Token 刷新失败: " + (responseBody.msg || "无法解析响应"));
    }
